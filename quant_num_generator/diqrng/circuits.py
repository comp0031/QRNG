...
class ChshCircuit:
    ...
    def __init__(
        self, num_pairs: int, token: str | None, angle: float = math.pi / 4
    ) -> None:
        if token:
            service = QiskitRuntimeService(channel="ibm_quantum", token=token)
            self._backend = service.least_busy(
                operational=True, simulator=False, min_num_qubits=num_pairs * 2
            )
    ...
    def _chsh_operator(self, base, i):
        ...
        return SparsePauliOp.from_list(
            [(_pad(A, A), 1), (_pad(A, B), 1), (_pad(B, A), -1), (_pad(B, B), 1)]
        )
